...
NAN_METHOD(Region::CreateSubRegion) {
  ...
  //   NanThrowError("createSubRegion: The region configuration object must have a type property.");
  //   NanReturnUndefined();
  // }
  ...
  const RegionAttributesPtr regionAttributesPtr(attributesFactory->createRegionAttributes());
...
NAN_METHOD(Region::GetSubRegion) {
  ...
  if (!args[0]->IsString()) {
    NanThrowError("You must pass a string as the name of a GemFire subregion to getSubRegion.");
    NanReturnUndefined();
  }
